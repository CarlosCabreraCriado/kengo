...
@Component({
  ...
  template: `
    <form class="composer" [class.composer--disabled]="disabled()" (submit)="onSubmit($event)">
      <input
  ...
  styles: [`
    ...
    .composer__input::placeholder { color: var(--ink-400); }
    .composer__send {
      width: 38px;
      height: 38px;
      flex-shrink: 0;
    ...
    .composer__send:hover:not(:disabled) { filter: brightness(1.05); }
    .composer__send:active:not(:disabled) { transform: translateY(1px); }
    .composer__send:disabled { opacity: 0.4; cursor: not-allowed; box-shadow: none; }
    .composer__send .material-symbols-outlined { font-size: 22px; font-weight: 700; }
  `],
})
export class ChatComposerComponent {
  ...
  onSubmit(event: Event): void {
    ...
    const input = target.querySelector('input.composer__input') as HTMLInputElement | null;
