...
- Assets fuente en `apps/app/assets/`: `splash.png` (**2732×2732**), `icon-only.png` (1024×1024, icono legacy e iOS) y, para el icono adaptativo de Android, `icon-foreground.png` (transparente) + `icon-background.png`. **Sin los dos últimos, `@capacitor/assets` no genera el icono adaptativo** y el launcher de Android 8+ se queda con el placeholder de Capacitor.
...
- Android: `values/styles.xml > AppTheme.NoActionBarLaunch` usa el API moderno `Theme.SplashScreen` (background = `@color/splashBackground` blanco, animated icon = `@drawable/splash_icon`, post = `AppTheme.NoActionBar`). Los colores están en `values/colors.xml`.
- **El splash de arranque de Android NO sale de `assets/splash.png`.** Sale de `res/drawable/splash_icon.xml`, un vector **cuadrado** hecho a mano con el isotipo de `src/assets/logo.svg`. Es obligatorio que sea cuadrado: `core-splashscreen` dibuja ese drawable en una caja de 288dp y lo recorta a un círculo de 192dp, así que un rectangular (como los `drawable*/splash.png` que genera `cap:assets`) sale **achatado** en Android 10 y recortado en Android 12+. **No renombrarlo a `splash`**: `cap:assets` sobrescribe ese nombre y el bug volvería en silencio. La geometría (viewport, translate, radio máximo) está documentada y calculada en la cabecera del propio XML; si cambia el logo, recalcularla ahí. Los `drawable*/splash.png` se conservan para el camino de fallback del plugin y están protegidos del shrinker en `res/raw/keep.xml`.
