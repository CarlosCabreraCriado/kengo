...
// ─── MIGRATE PASSWORD (migración transparente desde Directus) ───
// Cuando loginConvex falla pero loginDirectus OK, el cliente llama aquí con
// email+password para replicar el password en Better-Auth. No verifica Directus
// porque el cliente ya hizo ese paso vía loginDirectus y nosotros confiamos en
// el dual-source paralelo. Tras esta operación, el usuario queda marcado como
// passwordMigrated=true y futuros logins funcionan vía Convex puro.

http.route({
  path: "/api/auth/migrate-password",
  method: "OPTIONS",
  handler: optionsHandler(),
});
...
http.route({
  ...
  handler: httpAction(async (ctx, request) => {
    ...
    // Intento 1: signUp (crea cuenta Better-Auth + password hashed).
    try {
      await auth.api.signUpEmail({
        body: { email, password, name },
        headers: request.headers,
      });
      await ctx.runMutation(internal.auth.mutations.markPasswordMigrated, {
        email,
      });
      return new Response(
        JSON.stringify({ success: true, method: "signUp" }),
        { status: 200, headers },
      );
    } catch (signUpErr: any) {
      const errMsg = String(signUpErr?.message ?? signUpErr).toLowerCase();
      const yaExiste =
        errMsg.includes("already") ||
        errMsg.includes("exists") ||
        errMsg.includes("duplicate") ||
        errMsg.includes("user_already");

      if (!yaExiste) {
        console.error("[HTTP] migrate-password signUp error:", signUpErr);
        return new Response(
          JSON.stringify({
            success: false,
    ...
    // Intento 2: el usuario ya existe en Better-Auth (credencial vieja). Usamos
    // el patrón forget+reset para sobrescribir el password con el nuevo.
    try {
      clearPendingResetToken(email);
      await auth.api.requestPasswordReset({
        body: { email, redirectTo: "https://kengoapp.com/reset" },
      ...
      if (!resetToken) {
        return new Response(
          JSON.stringify({
            success: false,
            message: "No se generó token de reset",
          }),
          { status: 500, headers },
        );
      }
      ...
      console.error("[HTTP] migrate-password reset error:", resetErr);
      return new Response(
        JSON.stringify({
          success: false,
