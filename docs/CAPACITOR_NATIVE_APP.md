...
| Stripe en native | **CTAs de compra ocultos en builds nativos** (checkout, portal, reactivar, cambio de variante) vía `SubscriptionService.pagosSoloWeb` + guard defensivo en los métodos del servicio. En su lugar, mensajes de "gestión desde la versión web" (sin URL clicable, anti-steering). "Cancelar suscripción" sí se mantiene (las stores no exigen IAP para cancelar). | Políticas de pagos de Google Play (Play Billing + anti-steering) y Apple 3.1.1: ofrecer o enlazar el checkout de Stripe en la app es la causa de rechazo/retirada más habitual. Patrón multiplataforma (3.1.3(b)): usar en la app una sub comprada en la web es lícito. El flujo web queda intacto. |
| Retorno desde Stripe | Interstitial HTTPS (`/billing-return.html`) que dispara `kengo://billing/return?status=...` | Stripe rechaza esquemas custom en `success_url` / `cancel_url` — solo acepta HTTPS. El interstitial actúa como puente HTTPS→deep link. **Con los CTAs ocultos este flujo ya no se dispara en nativo; se conserva como fallback** por si algún día se activa el programa de ofertas externas (EEE/US). |
